...
class FineAdapter(private val items: List<Fine>) :
    RecyclerView.Adapter<FineAdapter.ViewHolder>() {

    class ViewHolder(val binding: ItemFineBinding) :
        RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemFineBinding.inflate(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with(holder.binding) {
            ...
            if (item.status == FineStatus.PENDENTE) {
                ...
                tvStatus.setBackgroundResource(R.drawable.bg_status_red) // Assumindo que criarei ou existe
            } else {
